...
bool is_base_format_used_by_rk_video(const uint32_t base_format)
{
	if ( MALI_GRALLOC_FORMAT_INTERNAL_NV12 == base_format
		...
		|| MALI_GRALLOC_FORMAT_INTERNAL_YUV444_10BIT_RFBC == base_format
					 ...
					 const int buffer_size) // Buffer resolution (w x h, in pixels).
{
	...
	else if ( HAL_PIXEL_FORMAT_YUV444_10BIT_RFBC == req_format )
	{
		D("to use MALI_GRALLOC_FORMAT_INTERNAL_YUV444_10BIT_RFBC as internal_format for HAL_PIXEL_FORMAT_YUV444_10BIT_RFBC.");
		internal_format = MALI_GRALLOC_FORMAT_INTERNAL_YUV444_10BIT_RFBC;
	}
